...
def extract_glyphs(scale, background):
    glyphs = []
    with Image(
        filename=font_file, resolution=source_dpi * scale, background=background
    ) as img:
    ...
    with Image(
        filename=logo_file, resolution=source_dpi * scale, background=background
    ) as img:
...
def generate_ws(output_path, scale):
    ...
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    with Image(
        width=scale * char_width,
        height=len(glyphs) * scale * char_height,
        background=ws_bg_color,
    ) as img:
